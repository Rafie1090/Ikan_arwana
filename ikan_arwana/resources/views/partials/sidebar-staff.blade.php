        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200 transform -translate-x-full {{ $isFixed ? 'md:translate-x-0 md:static' : '' }} transition-transform duration-300 z-50 flex flex-col">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-1">
                ...
                <a href="{{ route('staff.product.stock') }}" 
                   class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition
                          {{ request()->routeIs('staff.product.stock') ? 'bg-primary text-white shadow-lg shadow-indigo-500/30' : 'text-slate-600 hover:bg-slate-50 hover:text-primary' }}">
                    <i class="fa-solid fa-box-open w-5"></i> Kelola Stok Pnjualan
                </a>
            ...
            <div class="p-4 border-t border-slate-100">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 transition">
